...
public class RequestUtil {
    ...
    public static HttpServletRequest getHttpServletRequest(PortletRequest request) {
        try {
            Object req = request;
            while (!(req instanceof PortletRequestImpl)) {
                Method method = req.getClass().getMethod(GET_REQUEST);
                req = method.invoke(req);
            }
            return ((PortletRequestImpl) req).getHttpServletRequest();
