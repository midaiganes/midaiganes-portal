...
public class JsonUtil {
    ...
    static {
        gson = new GsonBuilder().disableInnerClassSerialization().registerTypeAdapter(ChatCmds.class, new ChatCmdsJsonSerializer())
                .registerTypeAdapter(MsgChatCmd.class, new MsgChatCmdJsonSerializer()).registerTypeAdapter(PrivMsgChatCmd.class, new PrivMsgChatCmdJsonSerializer())
    ...
    private static class ChatCmdJsonSerializer {
        ...
        private JsonElement serialize(ChatCmd<?> src, JsonSerializationContext context) {
            JsonObject jo = new JsonObject();
            Cmd cmd = src.getCmd();
