...
public class PortletApp {
    ...
    public PortletApp(PortletInstance portletInstance, PortletMode portletMode, WindowState windowState, PortletPreferencesRepository portletPreferencesRepository,
            PortletAndConfiguration portletConfiguration) {
    ...
    private RenderRequestImpl getRenderRequest(HttpServletRequest request, HttpServletResponse response, PortletPreferences portletPreferences) {
        return new RenderRequestImpl(getHttpServletRequestForRenderRequest(request), response, namespace, portletMode, windowState, portletPreferences, portletConfiguration);
    }

    private HttpServletRequest getHttpServletRequestForRenderRequest(HttpServletRequest request) {
        RenderParameterMap map = RenderParameterUtil.getRenderparameterMap(request, namespace);
        if (map != null) {
            return new RenderRequestParametersHttpServletRequest(request, map);
        }
        return request;
    }
